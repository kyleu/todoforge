...
3. `server.js` registers `sw.js`, which loads `todoforge.wasm`
...
- `./bin/build/wasmrelease.sh <version>`: Create `./build/dist/todoforge_<version>_wasm_html.zip`
- `./bin/wasmdev.sh`: Serve `./build/wasm` on `http://localhost:40009`
- `todoforge wasm`: Internal entrypoint used by the Service Worker (not for direct CLI use)
...
# upload build/dist/todoforge_1.2.3_wasm_html.zip to static hosting
```
